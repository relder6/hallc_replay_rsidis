...
def extract_numbers(lines, key):
    ...
    for line in lines:
        if line.startswith(key):
            capture = True
        if capture:
            # extract all numeric sequences (allow decimals)
            numbers = re.findall(r"[-+]?\d*\.\d+|\d+", line)
            values.extend([float(n) for n in numbers])
            # stop capturing if line does not end with comma
            if not line.rstrip().endswith(","):
                capture = False
    return values

pos_values = extract_numbers(lines, "pcal_pos_gain_cor")
neg_values = extract_numbers(lines, "pcal_neg_gain_cor")
arr_values = extract_numbers(lines, "pcal_arr_gain_cor")

# -----------------------------
# Compute replacement average
# -----------------------------
combined = np.array([v for v in pos_values + neg_values + arr_values if v > 0])
...
arr_fixed = [v if v > 0 else replacement_avg for v in arr_values]
...
with open(output_filename, "w") as out:
    ...
    for line in lines:
        ...
        if line.startswith("pcal_pos_gain_cor"):
            current = "pos"
            out.write(replace_numbers_in_line(line, pos_iter))
        elif line.startswith("pcal_neg_gain_cor"):
            current = "neg"
            out.write(replace_numbers_in_line(line, neg_iter))
        elif line.startswith("pcal_arr_gain_cor"):
            current = "arr"
            out.write(replace_numbers_in_line(line, arr_iter))
        elif current == "pos":
            out.write(replace_numbers_in_line(line, pos_iter))
        elif current == "neg":
            out.write(replace_numbers_in_line(line, neg_iter))
