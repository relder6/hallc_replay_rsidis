...
std::string anacuts = "abs(CTime.ePiCoinTime_ROC2)<150&&P.aero.npeSum>4&&H.cal.etottracknorm>0.7&&P.cal.etottracknorm<0.8&&H.cer.npeSum>1&&abs(P.gtr.dp-5.)<15.&&abs(H.gtr.dp)<8.";
...
{  
  ...
  if (is_50k) {
    ...
    pvtxt->AddText(Form("Rate of Good Coin Events : %.4f per CODA event", (double)counts[2]/50000.));
